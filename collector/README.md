...
- **追加リトライを`02:05`（日本時間11:05）にした理由**: GMOコインは毎週土曜9:00〜11:00（日本時間）に定例システムメンテナンスがある
  （https://support.coin.z.com/hc/ja/articles/115007815487）。通常の窓（日本時間09:00〜10:00）はメンテ時間内にすべて収まってしまうため、この間の取得は必ず失敗する。メンテ終了（11:00 JST）ちょうどだとAPIの復旧に数分かかる場合があるため、5分のバッファを見て`02:05 UTC`（11:05 JST）に設定している。GMO以外の一時的な取得失敗も、このスロットでまとめてリトライされる。
