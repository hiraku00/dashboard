...
class GmoCoinConnector(ExchangeConnector):
    ...
    def fetch(self, credentials: dict[str, str], account_name: str) -> list[dict]:
        ...
        if data.get("status") not in (0, None):
            ...
            details = []
            for message in messages[:3] if isinstance(messages, list) else []:
                if not isinstance(message, dict):
                    continue
                code = str(message.get("message_code") or "").strip()
                text = str(message.get("message_string") or "").strip()
                if code and text:
                    details.append(f"{code}: {text}")
                elif code or text:
                    details.append(code or text)
            detail = "; ".join(details) or f"status={data.get('status')}"
            raise ExchangeError(f"GMOコイン APIエラー: {detail}")
        rows = data.get("data", [])
        if not isinstance(rows, list):
            raise ExchangeError("GMOコイン APIの残高データ形式が不正です")
        positions = []
        for row in rows:
