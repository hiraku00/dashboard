...
DAILY_UPDATE_STATE_FILE = DATA / "daily-update-state.json"
STATIC = ROOT / "static"
LOCK = threading.Lock()
...
def daily_update_state() -> dict:
    if not DAILY_UPDATE_STATE_FILE.exists():
        return {}
    try:
        value = json.loads(DAILY_UPDATE_STATE_FILE.read_text(encoding="utf-8"))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/state":
            ...
            return json_response(self, {"wallets": wallets, "snapshots": snapshots, "sources": [source_public(x) for x in load_sources()], "exchange_snapshots": latest_portfolio_snapshots(), "daily_update": daily_update_state()})
        if path == "/api/history":
