...
function routineInput(body: Record<string, unknown>) {
  ...
  return { value: { title, description, scheduleType, weekdays, priority } };
}
...
export async function POST(request: Request) {
  ...
  const id = crypto.randomUUID(); const timestamp = now(); await env.DB.prepare("INSERT INTO todo_routines (id,board_id,title,description,priority,schedule_type,weekdays,default_column_id,active,version,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,1,1,?,?)").bind(id, BOARD_ID, normalized.value.title, normalized.value.description, normalized.value.priority, normalized.value.scheduleType, normalized.value.weekdays, defaultColumnId, timestamp, timestamp).run();
