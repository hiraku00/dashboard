 ...
 *  どの view で開いても資産概要/保管場所/通貨推移/データ更新が揃って表示できる
 *  よう、レガシーアプリの load() が起動時に読んでいたのと同じ4本＋最終同期
 *  情報をまとめて取る。 */
export async function fetchManageAssetInitial(): Promise<ManageAssetInitial | null> {
  ...
  try {
    ...
    return { state: state as unknown as AssetStateData, history: history as unknown as AssetHistoryData, lidoRewards: rewards, usdJpyRates: rates, latestSyncRun: run };
  } catch {
    return null;
  }
}
