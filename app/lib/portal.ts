...
export async function putPortalObject(args: { key: string; body: ArrayBuffer; category: string; contentType: string; sha?: string; expiresAt?: string | null }) {
  ...
  const storage = await currentStorageBytes();
  if (storage.bytes - oldBytes + args.body.byteLength > R2_SOFT_LIMIT_BYTES) throw new Error("R2の安全上限（8GB）に達するため保存を停止しました。古い原本を整理してください。");
  ...
  return { key: args.key, size: args.body.byteLength, sha256: digest, usedBytes: storage.bytes - oldBytes + args.body.byteLength };
}
