...
- **一覧**: Watch List と同じ表形式。1番組（1ノート）＝1行で、番組名を押すと詳細へ移る。
  - 列（短い名前）: 種別（スレッド／コメント）、放送局、放送タイトル（その日の放送。ここまでは人が編集する）、スレッド（LINEのスレッドの1行目と、内容の抜粋1行）、スレ主、投稿（日時。`yy/mm/dd hh:mm`、概算は「約」）、コメント全体（ノート全体のコメント数）、コメントちきりん（ちきりんのコメント数）、最新ちきりん（ちきりんの最新の投稿日時）、状態（OK／要確認。要確認は、コメント件数が表示と合わず再確認待ち、または本文が途中までの可能性。理由はホバーと詳細に出る）、リンク（編集したリンク＋ノートのリンクカード）。**一覧は読み取り専用**で、編集は詳細画面で行う。番組名の横の「新着」は、最後の取得で、ちきりんの投稿（スレッド・コメント）が初めて見つかった番組
