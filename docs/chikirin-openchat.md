...
- **一覧**: Watch List と同じ表形式。1番組（1ノート）＝1行で、番組名を押すと詳細へ移る。
  - 列（短い名前）: 種別（スレッド／コメント）、放送局、放送タイトル（その日の放送。ここまでは人が編集する）、スレッド（LINEのスレッドの1行目と、内容の抜粋1行）、スレ主、投稿（日時。`yy/mm/dd hh:mm`、概算は「約」）、コメント全体（ノート全体のコメント数）、コメントちきりん（ちきりんのコメント数）、最新ちきりん（ちきりんの最新の投稿日時）、状態（OK／要確認。要確認は、コメント件数が表示と合わず再確認待ち、または本文が途中までの可能性。理由はホバーと詳細に出る）、リンク（編集したリンク＋ノートのリンクカード）、編集ボタン
  - **編集**: 行の「編集」から、放送局・その日の放送タイトル・リンク（URLと表示名、5件まで）を保存する。`openchat_note_meta` に保存し、collector の同期では上書きされない。放送局・放送タイトルは検索の対象になる。
  ...
  - 画面の上に、最後の取得（collector が読み取りを始めた時刻。送信が遅れても取得の時刻）と、警告の件数・内容（`openchat_sync_runs` の最新。押すと警告の一覧）を表示する。**日時はすべて日本時間（JST, UTC+9）**で、画面にも明記する。
