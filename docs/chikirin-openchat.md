...
- **一覧**: Watch List と同じ表形式。1番組（1ノート）＝1行で、番組名を押すと詳細へ移る。
  - 列（短い名前）: 種別（スレッド／コメント）、放送局、放送タイトル（その日の放送。ここまでは人が編集する）、スレッド（LINEのスレッドの1行目と、内容の抜粋1行）、スレ主、投稿（日時。`yy/mm/dd hh:mm`、概算は「約」）、コメ（ノート全体のコメント数）、ちきりん（ちきりんのコメント数）、最新（ちきりんの最新の投稿日時）、リンク（編集したリンク＋ノートのリンクカード）、編集ボタン
...
- **詳細** `/chikirin/:id`: 番組名、スレッド主・投稿日時・コメント数、リンクカード、スレッド主の投稿（番組の情報。ちきりんのスレッドなら「ちきりんのスレッド」）、ちきりんのコメントを古い順に全文で（各コメントに日時。本人が立てたノートへのコメントなら「本人コメント」）。一覧に載らないノート・存在しないノートは「見つかりません」。`GET /api/openchat/programs/:id` と同じ `getProgram` を使う。
- 他の人のコメントは表示しない。
