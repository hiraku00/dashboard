...
LINEオープンチャット「集まれテレビっ子」のノート（1ノート＝1番組）から、ちきりんさんの投稿を取り出してポータルに保存・表示する機能の設計。Watch List / TextTube と並ぶ新機能として実装する。
...
| 対象 | 保存 | 画面表示 |
|---|---|---|
| ちきりんさんが立てたノートの本文（全文） | する | する |
| どのノートでも、ちきりんさんが書いたコメント（1ノートに複数あれば全部） | する | する |
| ちきりんさん以外が立てたノートの情報（投稿者・日時・本文・コメント数） | する | しない（取得済みかどうかの判定用） |
| ちきりんさん以外のコメント | する | しない（重複判定・件数照合・削除検知用） |

ちきりんさん以外の投稿を保存するのは、差分取得を正確にするため（ユーザー了承済み）。画面にもAPIの一覧にも出さない。

### ちきりんさんが関わる場合の組み合わせ

1つのノート（番組）について、次のどれも起こりうる前提で設計する。

1. **他の人のノート**に、ちきりんさんがコメントを1回書く
2. **他の人のノート**に、ちきりんさんがコメントを**複数回**書く（間に他の人のコメントが入ることもある）
3. **ちきりんさんが立てたノート**で、コメントはない
4. **ちきりんさんが立てたノート**に、本人がさらにコメントを1回以上書く（補足・返信）
5. どちらもなし → 画面には出さない（保存のみ）

画面では1ノート＝1番組のカードにまとめ、「ちきりんさんの本文（3・4のとき）」と「ちきりんさんのコメント（投稿時刻の古い順に全部）」を並べる。
...
Cloudflare Worker
  ...
  GET  /api/openchat/programs        … 画面用一覧（ちきりんさんの投稿があるノートだけ）
...
| 公式バッジ（ちきりんさん判定） | アバターの下側45%〜下端+6pt、x=32〜49 の範囲に、青い画素（R<40, 90≤G≤180, B≥235）が40pt²以上。実機では `#0070FF` の単色と `#00A4FF→#0096FF` の縦グラデーションの両方があった |
...
| 「もっと見る」 | ノート本文の続きを開くボタン。ちきりんさんのノートの場合だけ押す（x=60, 該当行のy） |
...
| スクロール | ピクセル単位のスクロールイベントは無視されるので、行単位で送る（`kCGScrollEventUnitLine`、1回あたり3行）。移動量が一定しないので、画面が重なっているかを毎回確認する |

### 画面の重なり確認と、長いブロックのつなぎ合わせ

- スクロールの前後で、完全なブロックが1つも共通していなければ `gap` と判定し、半分の量だけ戻して撮り直す（最大3回）。それでも重ならなければ、そのノートを `needs_recheck` にする。
- 1つのコメントが画面の高さ（1130pt）を超える場合は、作者行はあるのに時刻行が画面内に無い状態になる。このときはスクロール量を半分にして、次の画面の上端にある不完全なブロックの行と、前の画面の末尾の行を**行単位の重なり**（末尾k行と先頭k行の一致。k=2〜5）でつなぎ合わせる。
...
ちきりんさんが**同じノートに複数回コメントする**ケースを、1つにまとめてしまわないための規則。
...
- 本文が違えば、同じ作者でも別のコメントになる。ちきりんさんの2件目・3件目も、それぞれ別に保存される。
...
- 一覧の最後は「＜重要＞このオープンチャットは…」のノート（9.21 午後2:17、ちきりんさん投稿。検証では、ここより古いノートは見つからなかった）と想定している。これを見たら終了する。**一覧の本当の末尾かどうかは未確認**なので、実装の最初の手順で確かめる。

### `open_and_collect(note)`

1. ちきりんさんのノートで、本文に「もっと見る」があれば押して全文を読む。
...
4. 下へスクロールしながら、`__END__`（コメント入力欄）まで、コメントを表示順に集める（§4 のつなぎ合わせと重なり確認を行う）。
5. **件数の照合**: 集めた件数が一覧のコメント数と違えば、3に戻って1回だけやり直す。それでも違えば `needs_recheck=1` にし、そのノートの `comment_count` は更新しない（次回もまた開く）。集めた分は保存する。
...
CREATE TABLE IF NOT EXISTS openchat_notes (
  ...
  author_is_target INTEGER NOT NULL DEFAULT 0,   -- ちきりんさんのノートか
  ...
  body_text TEXT NOT NULL DEFAULT '',            -- ちきりんさんのノートは全文、それ以外は画面に見えている範囲
  ...
  target_comment_count INTEGER NOT NULL DEFAULT 0, -- ちきりんさんのコメント数（一覧表示用に保持）
...
```jsonc
// start
{ "action": "start", "clientRunId": "2026-09-24T12:00:00+07:00-ab12", "clientVersion": "line-openchat/1" }
...
### `GET /api/openchat/programs?q=&kind=all|thread|comment&cursor=`

画面用。ちきりんさんの投稿があるノートだけを、`posted_at` の新しい順に返す（1ページ20件）。

- `kind=thread`: ちきりんさんが立てたノートだけ
- `kind=comment`: ちきりんさんのコメントがあるノートだけ（本人が立てたノートでも、本人のコメントがあれば含む）
- `q`: 番組名（`program_title` / `link_title`）と、ちきりんさんの本文・コメントの部分一致
...
- 1ノート＝1枚のカード:
  - 見出し: 番組名（`program_title`、リンクカードがあれば題名とリンク）
  - サブ行: ノートの投稿者・投稿日時（approx なら「約」）・コメント数
  - ちきりんさんが立てたノートなら「ちきりんさんのスレッド」の目印と本文（長ければ折りたたむ）
  - ちきりんさんのコメントを古い順に全部（各コメントに日時）。本人が立てたノートへのコメントなら「本人コメント」と表示する
- 絞り込み: すべて／本人スレッド／コメント。キーワード検索。
- 画面の上に、最後の同期時刻と警告の件数（`openchat_sync_runs` の最新）を表示する。
- 他の人のコメントは表示しない。
...
| 画面が重ならない（スクロールの飛び） | 半分戻して撮り直し（最大3回）→ `needs_recheck=1` |
| バッジと名前が合わない | 警告に記録（§5） |
...
- **実機確認**: 初回の全件取得のあと、ちきりんさんのコメントがあるノートを3件選び、画面上の件数・本文と保存内容を目で照合する。
...
- **送信は小さく分ける**: 1リクエスト10ノート・40コメントまで（Workers FreeのCPU上限のため）。1つのノートのコメントが多いときは、ノートの情報を繰り返しながらコメントを分けて送る。Worker側は最後にちきりんさんのコメント数を数え直す。`start` は再送されても同じ実行を返す。
