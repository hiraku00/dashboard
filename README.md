...
- **ちきりんオプチャ**: LINEオープンチャット「集まれテレビっ子」のノート（1ノート＝1番組）から、ちきりんが立てたスレッドと、ちきりんのコメントだけを番組ごとに表示。Mac上のcollectorがLINEの画面を読み取り（参照のみ）、差分だけをD1へ同期
...
| `/chikirin` | ちきりんオプチャ。番組ごとの一覧（表・ページ移動・検索・絞り込み・放送局や放送タイトル、リンクの編集） |
| `/chikirin/:id` | ちきりんオプチャの詳細。スレッド主の投稿（番組の情報）とちきりんのコメントを全文で表示 |
